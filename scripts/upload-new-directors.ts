...
class NewDirectorsUploader {
  ...
  async createVideoEntry(videoData: VideoData): Promise<any> {
    ...
    try {
      ...
      if (videoData.thumbnailId) {
        entryData.fields.thumbnailId = {
          'en-US': videoData.thumbnailId
        };
      }
      
      const entry = await this.environment.createEntry('directorVideo', entryData);
  ...
  async uploadDirectorsToContentful(directorsData: DirectorData[]): Promise<void> {
    ...
    for (const directorData of directorsData) {
      try {
        console.log(`\n🎬 Processing director: ${directorData.name}`);
        ...
        for (const videoData of directorData.videos) {
          const videoEntry = await this.createVideoEntry(videoData);
        ...
        console.log(`✅ Successfully uploaded director: ${directorData.name} with ${videoEntries.length} videos`);
...
async function main() {
  ...
  console.log('🎬 Bristol New Directors Upload Script');
  console.log('======================================');
  ...
  const uploader = new NewDirectorsUploader();
  ...
  try {
    ...
    console.log('- Directors without videos will be skipped');
